...
/*$sql="SELECT sm.Sales_Master_Id,sm.Serial_No,DATE_FORMAT(sm.Invoice_Date,'%d/%m/%Y') AS Invoice_Date,sm.Invoice_No,sm.Customer_Name,sm.IsCash,CASE
...
$sql="SELECT sm.Sales_Master_Id,DATE_FORMAT(sm.Invoice_Date,'%d/%m/%Y') AS Invoice_Date,sum(sm.Total_Gross_Amount),sum(sm.Total_Discount_Amount),sum(sm.Total_Net_Amount),sum(sm.Total_Tax_Amount),sum(sm.Total_Amount) 
FROM `sales_master` sm 
left join customer sc on sm.Customer_Id=sc.Customer_Id where 1=1
$where group by sm.Invoice_Date order by sm.Sales_Master_Id desc ";

error_log("SELECT sm.Sales_Master_Id,sm.Serial_No,DATE_FORMAT(sm.Invoice_Date,'%d/%m/%Y') AS Invoice_Date,sum(sm.Total_Gross_Amount),sum(sm.Total_Discount_Amount),sum(sm.Total_Net_Amount),sum(sm.Total_Tax_Amount),sum(sm.Total_Amount) 
FROM `sales_master` sm 
left join customer sc on sm.Customer_Id=sc.Customer_Id where 1=1
$where group by sm.Invoice_Date order by sm.Sales_Master_Id desc ");
...
foreach ($saleslist_all as $k => $sales) {
	...
	$sum_discount_amount += $sales['Total_Discount_Amount'];
...
$sales_detail_list = $crud->getData($sql);
                                    if ($sales_detail_list){
									  echo '<tr class="stbl stbl_th sub_row  aa_'.$sales['Sales_Master_Id'].'">
									  <th>Product Code</th>
									  <th>Product Name</th>
									  <th>Brand Name</th>
									  <th>Quantity</th>
									 ...
									 echo  '<tr class="stbl sub_row  aa_'.$sales['Sales_Master_Id'].'">
								...
								echo  '<tr>
									  ...
									  <td><b>'.$sales_report_details['sum_total_amount'].'</b></td></tr>';
							} else {
