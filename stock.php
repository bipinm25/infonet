...
if (isset($_GET['unset_filter']) && $_GET['unset_filter']==1) {
	...
	if (isset($_GET['category_id']) && !empty($_GET['category_id'])) {
		$where.=" and pm.Category_ID = ".(int) $_GET['category_id'];
	}
	if (isset($_GET['brand_id']) && !empty($_GET['brand_id'])) {
		$where.=" and pm.Brand_ID = ".(int) $_GET['brand_id'];
	}
}

$product = $crud->getData("select Product_Detail_Id,Product_Detail_Name from product_detail order by Product_Detail_Name");

$product_category = $crud->getData("select Category_ID,Category_Name from product_category order by Category_Name");

$product_brand = $crud->getData("select Brand_ID,Brand_Name from product_brand order by Brand_Name");
...
$sql="SELECT pd.Product_Detail_Id,pd.Product_Id,pd.Product_Code,pd.Product_Detail_Name,pb.Brand_Name,pc.Category_Name,pm.Opening_Stock,sd.sales_quantity,srd.sales_return_quantity,pud.purchase_quantity,purd.purchase_return_quantity,st.Adjust_Add,st.Adjust_Minus,pd.Stock, pd.Purchase_Cost,(pd.Stock*pd.Purchase_Cost) AS stock_value 
...
<section class="panel panel-default">
    ...
    <div class="col-lg-2">
			...
			<option value="">Category</option>
				<?php foreach ($product_category as $k => $category) {
					$select_cat='';
					if (isset($_GET['category_id'])) {
						$select_cat = (int) $_GET['category_id'] == $category['Category_ID']? 'selected="selected"':"" ;
					}						
					echo '<option '.$select_cat.' value="'.$category['Category_ID'].'">'.$category['Category_Name'].'</option>';
					}
				?>				
			</select>
	</div>
    ...
    <div class="col-lg-2">
			...
			<option value="">Brand</option>
				<?php foreach ($product_brand as $k => $brand) {
					$select_bra='';
					if (isset($_GET['brand_id'])) {
						$select_bra = (int) $_GET['brand_id'] == $brand['Brand_ID']? 'selected="selected"':"" ;
					}						
					echo '<option '.$select_bra.' value="'.$brand['Brand_ID'].'">'.$brand['Brand_Name'].'</option>';
					}
				?>				
			</select>
	</div>
		...
		$html .='<table '.$pdf_tbl.'  class="table b-t b-light table-striped">
			...
			<tbody>';
							...
							if ($stock_list){
								...
								<td '.$pdf_td.'>'.$stock['Category_Name'].'</td>
								<td '.$pdf_td.'>'.$stock['Brand_Name'].'</td>
